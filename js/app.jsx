    ...
    <div className="page-layout" style={{ maxWidth: 1040, margin: '0 auto', padding: '40px 24px', display: 'block', gap: 48, alignItems: 'flex-start' }}>
      <main style={{ flex: 1, minWidth: 0 }}>
        ...
        <div>
          ...
          <ResearchArea title="Privacy-Preserving Formal Methods" url="#" thumb="assets/privacy.png" projects={allPubs.filter(p => (p.themes || []).includes('privacy') && p.pdf && p.pdf !== '#').sort((a, b) => b.year - a.year).slice(0, 4).map(p => ({ title: p.title, url: p.pdf && p.pdf !== '#' ? p.pdf : '#' }))}>
            Formal reasoning normally requires seeing everything involved in the task: the program, the inputs, and the specification. Cryptography lets us drop that requirement, so that one party can prove a program satisfies a property while revealing nothing beyond the claim itself, or several parties can check whether their policies conflict and jointly search for a configuration satisfying all of them, without any one of them revealing the details it is trying to protect. 
            The work spans zero-knowledge proof systems for automated reasoning (proving unsatisfiability, SMT theorems, and regular-expression equivalence in zero knowledge) and secure multi-party protocols where mutually distrusting parties jointly solve constraint problems over inputs neither will disclose. Recent directions include privacy-preserving SAT and MaxSAT solving, delegated computation on randomness, and protocols for verifying properties of proprietary AI models without exposing their weights.
          </ResearchArea>

          <ResearchArea title="ML/AI + Formal Methods" url="#" thumb="assets/mlai.png" projects={allPubs.filter(p => (p.themes || []).includes('mlai') && p.pdf && p.pdf !== '#').sort((a, b) => b.year - a.year).slice(0, 4).map(p => ({ title: p.title, url: p.pdf }))}>
            Machine learning and formal methods have complementary blind spots. Formal methods give guarantees but rely on search procedures whose guidance has been hand-tuned for decades: which variable a SAT solver should branch on, which candidate invariant to try next, how to decompose a proof obligation. Learning is good at exactly these decisions, since they are data-rich guesses with no clean theory behind them. Machine learning, in turn, produces artifacts that work well on average but come with no guarantees about any particular input, which is the gap formal reasoning is built to close. We build frameworks where each side supplies what the other lacks, i.e., learning does the guessing, formal reasoning does the checking.
          </ResearchArea>

          <ResearchArea title="Neurosymbolic AI" url="#" thumb="assets/neuro.png" projects={allPubs.filter(p => (p.themes || []).includes('neuro') && p.pdf && p.pdf !== '#').sort((a, b) => b.year - a.year).slice(0, 4).map(p => ({ title: p.title, url: p.pdf && p.pdf !== '#' ? p.pdf : '#' }))}>
            Some problems yield to neither pure learning nor pure symbolic reasoning. Neural methods handle perception, ambiguity, and open-ended search, while symbolic methods supply structure, soundness, and explanation. We combine them on problems where both are needed at once, e.g., agentic pipelines that propose environmental data proxies and then produce checkable arguments for why they hold, program synthesis and reachability analysis for models that biologists can trust mathematically, specification mining from execution data, and reasoning systems that carry this approach into law and policy.
          </ResearchArea>

          <ResearchArea title="Verification and Synthesis" url="#" thumb="assets/verification.png" projects={allPubs.filter(p => ((p.themes || []).includes('network') || (p.themes || []).includes('verification') || (p.themes || []).includes('synthesis')) && p.pdf && p.pdf !== '#').sort((a, b) => b.year - a.year).slice(0, 6).map(p => ({ title: p.title, url: p.pdf && p.pdf !== '#' ? p.pdf : '#' }))}>
            Verification asks whether a program meets its specification; synthesis asks the machine to produce a program that does. Both reduce to hard questions in logic and automated reasoning, and progress comes from better decision procedures, better proof techniques, and better ways of stating what correctness means. Work here includes relational and hyperproperty verification (proving things about pairs of executions rather than single runs), termination and non-termination proofs, separation logic for heap-manipulating programs, reactive and syntax-guided synthesis, program repair, and decision procedures for sets, multisets, and cardinality constraints. The same machinery applies well beyond conventional programs, to firewalls, routing policies, data-plane implementations, cloud service dependencies, and infrastructure-as-code, which are all software in the end, but written in idiosyncratic languages, rarely tested, and catastrophic when misconfigured.
          </ResearchArea>

          <ResearchArea title="Formal Methods for Accountable Decision-Making" url="#" thumb="assets/law.png" projects={allPubs.filter(p => (p.themes || []).includes('law') && p.pdf && p.pdf !== '#').sort((a, b) => b.year - a.year).slice(0, 4).map(p => ({ title: p.title, url: p.pdf && p.pdf !== '#' ? p.pdf : '#' }))}>
...
const TeamPage = ({ tweaks }) => {
  ...
  const alumni = [
    ...
    { name: 'Ennan Zhai', dest: 'Director of AI System Research at Alibaba Group', year: 'PostDoc 2018' , url: 'https://ennanzhai.github.io'},
