    ...
    <div className="page-layout" style={{ maxWidth: 1040, margin: '0 auto', padding: '40px 24px', display: 'block', gap: 48, alignItems: 'flex-start' }}>
      <main style={{ flex: 1, minWidth: 0 }}>
        ...
        <div>
          ...
          <ResearchArea title="Privacy-Preserving Formal Methods" url="#" thumb="privacy.png" projects={allPubs.filter(p => (p.themes || []).includes('privacy') && p.pdf && p.pdf !== '#').sort((a, b) => b.year - a.year).slice(0, 4).map(p => ({ title: p.title, url: p.pdf && p.pdf !== '#' ? p.pdf : '#' }))}>
          ...
          <ResearchArea title="ML/AI + Formal Methods" url="#" thumb="mlai.png" projects={allPubs.filter(p => (p.themes || []).includes('mlai') && p.pdf && p.pdf !== '#').sort((a, b) => b.year - a.year).slice(0, 4).map(p => ({ title: p.title, url: p.pdf }))}>
          ...
          <ResearchArea title="Neurosymbolic AI" url="#" thumb="neuro.png" projects={allPubs.filter(p => (p.themes || []).includes('neuro') && p.pdf && p.pdf !== '#').sort((a, b) => b.year - a.year).slice(0, 4).map(p => ({ title: p.title, url: p.pdf && p.pdf !== '#' ? p.pdf : '#' }))}>
          ...
          <ResearchArea title="Verification and Synthesis" url="#" thumb="verification.png" projects={allPubs.filter(p => ((p.themes || []).includes('network') || (p.themes || []).includes('verification') || (p.themes || []).includes('synthesis')) && p.pdf && p.pdf !== '#').sort((a, b) => b.year - a.year).slice(0, 6).map(p => ({ title: p.title, url: p.pdf && p.pdf !== '#' ? p.pdf : '#' }))}>
          ...
          <ResearchArea title="Formal Methods for Accountable Decision-Making" url="#" thumb="law.png" projects={allPubs.filter(p => (p.themes || []).includes('law') && p.pdf && p.pdf !== '#').sort((a, b) => b.year - a.year).slice(0, 4).map(p => ({ title: p.title, url: p.pdf && p.pdf !== '#' ? p.pdf : '#' }))}>
