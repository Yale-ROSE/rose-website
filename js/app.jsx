    ...
    <div className="page-layout" style={{ maxWidth: 1040, margin: '0 auto', padding: '40px 24px', display: 'block', gap: 48, alignItems: 'flex-start' }}>
      <main style={{ flex: 1, minWidth: 0 }}>
        ...
        <div>
          ...
          <ResearchArea title="Verification and Synthesis" url="#" thumb="" projects={allPubs.filter(p => ((p.themes || []).includes('network') || (p.themes || []).includes('verification') || (p.themes || []).includes('synthesis')) && p.pdf && p.pdf !== '#').sort((a, b) => b.year - a.year).slice(0, 4).map(p => ({ title: p.title, url: p.pdf && p.pdf !== '#' ? p.pdf : '#' }))}>
